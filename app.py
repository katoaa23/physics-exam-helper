# Шаг 1: Импорт библиотек
from flask import Flask, render_template, request, jsonify
import math

app = Flask(__name__)

# Шаг 2: Хранение формул с картинками (оставляем как было для фронтенда)
formulas = [
    ('Кинематика', 'Путь при равномерном движении', ['static/images/f1.jpg']),
    ('Кинематика', 'Средняя скорость', ['static/images/f2.jpg']),
    ('Кинематика', 'Ускорение при равноускоренном движении', ['static/images/f3.jpg']),
    ('Кинематика', 'Зависимость скорости от времени', ['static/images/f4.jpg']),
    ('Кинематика', 'Перемещение при равноускоренном движении', ['static/images/f5(1).jpg', 'static/images/f5(2).jpg', 'static/images/f5(3).jpg']),
    ('Кинематика', 'Максимальная высота при вертикальном броске', ['static/images/f6(1).jpg', 'static/images/f6(2).jpg', 'static/images/f6(3).jpg']),
    ('Кинематика', 'Время подъема', ['static/images/f7.jpg']),
    ('Кинематика', 'Полное время полета', ['static/images/f8.jpg']),
    ('Динамика', 'Второй закон Ньютона', ['static/images/f9.jpg']),
    ('Динамика', 'Третий закон Ньютона', ['static/images/f10.jpg']),
    ('Динамика', 'Сила упругости', ['static/images/f11.jpg']),
    ('Динамика', 'Закон всемирного тяготения', ['static/images/f12.jpg']),
    ('Динамика', 'Первая космическая скорость', ['static/images/f13.jpg']),
    ('Статика', 'Момент силы определяется с помощью следующей формулы', ['static/images/f14.jpg']),
    ('Статика', 'Условие при котором тело не будет вращаться', ['static/images/f15.jpg']),
    ('Гидростатика', 'Определение давления задаётся следующей формулой', ['static/images/f16.jpg']),
    ('Гидростатика', 'Давление, которое создает столб жидкости находится по формуле', ['static/images/f17.jpg']),
    ('Гидростатика', 'Идеальный гидравлический пресс', ['static/images/f18.jpg']),
    ('Гидростатика', 'Любой гидравлический пресс', ['static/images/f19.jpg']),
    ('Гидростатика', 'Сила Архимеда', ['static/images/f20.jpg']),
    ('Импульс', 'Импульс тела', ['static/images/f21.jpg']),
    ('Импульс', 'Изменение импульса тела или системы тел', ['static/images/f22.jpg']),
    ('Импульс', 'Второй закон Ньютона в импульсной форме', ['static/images/f23.jpg']),
    ('Импульс', 'Закон сохранения импульса', ['static/images/f24.jpg']),
    ('Работа, мощность, энергия', 'Работа', ['static/images/f25.jpg']),
    ('Работа, мощность, энергия', 'Мощность', ['static/images/f26.jpg']),
    ('Работа, мощность, энергия', 'Кинетическая энергия', ['static/images/f27.jpg']),
    ('Работа, мощность, энергия', 'Потенциальная энергия', ['static/images/f28.jpg']),
    ('Работа, мощность, энергия', 'Коэффициент полезного действия', ['static/images/f29.jpg']),
    ('Термодинамика', 'Первый закон термодинамики', ['static/images/f30.jpg']),
    ('Термодинамика', 'Количество теплоты', ['static/images/f31.jpg']),
    ('Термодинамика', 'Работа идеального газа', ['static/images/f32.jpg']),
    ('Электростатика', 'Закон Кулона', ['static/images/f33.jpg']),
    ('Электростатика', 'Напряженность электрического поля', ['static/images/f34.jpg']),
    ('Электростатика', 'Электрическое напряжение', ['static/images/f35.jpg']),
    ('Электрический ток', 'Закон Ома', ['static/images/f36.jpg']),
    ('Электрический ток', 'Работа электрического тока', ['static/images/f37.jpg']),
    ('Электрический ток', 'Сила тока', ['static/images/f38.jpg']),
    ('Магнетизм', 'Сила Ампера', ['static/images/f39.jpg']),
    ('Магнетизм', 'ЭДС индукции', ['static/images/f40.jpg']),
    ('Магнетизм', 'Сила Лоренца', ['static/images/f41.jpg']),
    ('Оптика', 'Закон преломления', ['static/images/f42.jpg']),
    ('Оптика', 'Формула тонкой линзы', ['static/images/f43.jpg']),
    ('Оптика', 'Оптическая длина', ['static/images/f44.jpg']),
    ('Атомная и ядерная физика', 'Энергия кванта', ['static/images/f45.jpg']),
    ('Атомная и ядерная физика', 'Закон радиоактивного распада', ['static/images/f46.jpg']),
    ('Атомная и ядерная физика', 'Второй постулат Бора или правило частот', ['static/images/f47.jpg']),
    ('Основы специальной теории относительности', 'Релятивистская масса', ['static/images/f48.jpg']),
    ('Основы специальной теории относительности', 'Полная энергия', ['static/images/f49.jpg']),
    ('Основы специальной теории относительности', 'Энергия покоя тела', ['static/images/f50.jpg']),
]

# Шаг 3: Новая система хранения формул (УНИВЕРСАЛЬНАЯ)
# Теперь каждая формула — это словарь с метаданными, а не отдельная функция
formula_definitions = {
    # ========== КИНЕМАТИКА ==========
    'Путь при равномерном движении': {
        'formula': 's = v × t',  # Отображаемая формула
        'parameters': ['v', 't'],  # Какие параметры нужны
        'calculation': 'v * t',  # Как вычислять (простая строка!)
        'units': {'v': 'м/с', 't': 'с', 'result': 'м'},  # Единицы измерения
        'description': 'Расчет пути при постоянной скорости'
    },
    
    'Средняя скорость': {
        'formula': 'v_ср = s / t',
        'parameters': ['s', 't'],
        'calculation': 's / t',
        'units': {'s': 'м', 't': 'с', 'result': 'м/с'}
    },
    
    'Ускорение при равноускоренном движении': {
        'formula': 'a = (v - v₀) / t',
        'parameters': ['v', 'v0', 't'],
        'calculation': '(v - v0) / t',
        'units': {'v': 'м/с', 'v0': 'м/с', 't': 'с', 'result': 'м/с²'}
    },
    
    'Зависимость скорости от времени': {
        'formula': 'v = v₀ + a × t',
        'parameters': ['v0', 'a', 't'],
        'calculation': 'v0 + a * t',
        'units': {'v0': 'м/с', 'a': 'м/с²', 't': 'с', 'result': 'м/с'}
    },
    
    'Перемещение при равноускоренном движении': {
        'formula': 's = v₀×t + (a×t²)/2',
        'parameters': ['v0', 'a', 't'],
        'calculation': 'v0 * t + 0.5 * a * t**2',
        'units': {'v0': 'м/с', 'a': 'м/с²', 't': 'с', 'result': 'м'}
    },
    
    'Максимальная высота при вертикальном броске': {
        'formula': 'h_max = v₀² / (2 × g)',
        'parameters': ['v0', 'g'],
        'calculation': 'v0**2 / (2 * g)',
        'units': {'v0': 'м/с', 'g': 'м/с²', 'result': 'м'}
    },
    
    'Время подъема': {
        'formula': 't_под = v₀ / g',
        'parameters': ['v0', 'g'],
        'calculation': 'v0 / g',
        'units': {'v0': 'м/с', 'g': 'м/с²', 'result': 'с'}
    },
    
    'Полное время полета': {
        'formula': 't_пол = 2 × v₀ / g',
        'parameters': ['v0', 'g'],
        'calculation': '2 * v0 / g',
        'units': {'v0': 'м/с', 'g': 'м/с²', 'result': 'с'}
    },
    
    # ========== ДИНАМИКА ==========
    'Второй закон Ньютона': {
        'formula': 'F = m × a',
        'parameters': ['m', 'a'],
        'calculation': 'm * a',
        'units': {'m': 'кг', 'a': 'м/с²', 'result': 'Н'}
    },
    
    'Третий закон Ньютона': {
        'formula': 'F₁₂ = -F₂₁',
        'parameters': ['F'],
        'calculation': '-F',  # Просто меняем знак
        'units': {'F': 'Н', 'result': 'Н'},
        'note': 'Силы равны по модулю и противоположны по направлению'
    },
    
    'Сила упругости': {
        'formula': 'F_упр = k × x',
        'parameters': ['k', 'x'],
        'calculation': 'k * x',
        'units': {'k': 'Н/м', 'x': 'м', 'result': 'Н'}
    },
    
    'Закон всемирного тяготения': {
        'formula': 'F = G × (m₁ × m₂) / r²',
        'parameters': ['G', 'm1', 'm2', 'r'],
        'calculation': 'G * m1 * m2 / (r**2)',
        'units': {'G': 'Н·м²/кг²', 'm1': 'кг', 'm2': 'кг', 'r': 'м', 'result': 'Н'}
    },
    
    'Первая космическая скорость': {
        'formula': 'v₁ = √(g × R)',
        'parameters': ['g', 'R'],
        'calculation': 'sqrt(g * R)',  # Используем sqrt из math!
        'units': {'g': 'м/с²', 'R': 'м', 'result': 'м/с'}
    },
    
    # ========== СТАТИКА ==========
    'Момент силы определяется с помощью следующей формулы': {
        'formula': 'M = F × l',
        'parameters': ['F', 'l'],
        'calculation': 'F * l',
        'units': {'F': 'Н', 'l': 'м', 'result': 'Н·м'}
    },
    
    'Условие при котором тело не будет вращаться': {
        'formula': 'ΣM = 0',
        'parameters': ['M1', 'M2', 'M3'],  # Пример: сумма моментов
        'calculation': 'M1 + M2 + M3',
        'units': {'M1': 'Н·м', 'M2': 'Н·м', 'M3': 'Н·м', 'result': 'Н·м'},
        'note': 'Сумма моментов сил должна быть равна 0'
    },
    
    # ========== ГИДРОСТАТИКА ==========
    'Определение давления задаётся следующей формулой': {
        'formula': 'p = F / S',
        'parameters': ['F', 'S'],
        'calculation': 'F / S',
        'units': {'F': 'Н', 'S': 'м²', 'result': 'Па'}
    },
    
    'Давление, которое создает столб жидкости находится по формуле': {
        'formula': 'p = ρ × g × h',
        'parameters': ['rho', 'g', 'h'],
        'calculation': 'rho * g * h',
        'units': {'rho': 'кг/м³', 'g': 'м/с²', 'h': 'м', 'result': 'Па'}
    },
    
    'Идеальный гидравлический пресс': {
        'formula': 'F₁ / F₂ = S₁ / S₂',
        'parameters': ['F1', 'S1', 'S2'],  # Находим F2
        'calculation': 'F1 * S2 / S1',
        'units': {'F1': 'Н', 'S1': 'м²', 'S2': 'м²', 'result': 'Н'}
    },
    
    'Любой гидравлический пресс': {
        'formula': 'F₁ / S₁ = F₂ / S₂',
        'parameters': ['F1', 'S1', 'S2'],
        'calculation': 'F1 * S2 / S1',
        'units': {'F1': 'Н', 'S1': 'м²', 'S2': 'м²', 'result': 'Н'}
    },
    
    'Сила Архимеда': {
        'formula': 'F_A = ρ × g × V',
        'parameters': ['rho', 'g', 'V'],
        'calculation': 'rho * g * V',
        'units': {'rho': 'кг/м³', 'g': 'м/с²', 'V': 'м³', 'result': 'Н'}
    },
    
    # ========== ИМПУЛЬС ==========
    'Импульс тела': {
        'formula': 'p = m × v',
        'parameters': ['m', 'v'],
        'calculation': 'm * v',
        'units': {'m': 'кг', 'v': 'м/с', 'result': 'кг·м/с'}
    },
    
    'Изменение импульса тела или системы тел': {
        'formula': 'Δp = p₂ - p₁',
        'parameters': ['p2', 'p1'],
        'calculation': 'p2 - p1',
        'units': {'p1': 'кг·м/с', 'p2': 'кг·м/с', 'result': 'кг·м/с'}
    },
    
    'Второй закон Ньютона в импульсной форме': {
        'formula': 'F = Δp / Δt',
        'parameters': ['delta_p', 'delta_t'],
        'calculation': 'delta_p / delta_t',
        'units': {'delta_p': 'кг·м/с', 'delta_t': 'с', 'result': 'Н'}
    },
    
    'Закон сохранения импульса': {
        'formula': 'p₁ + p₂ = p₁\' + p₂\'',
        'parameters': ['p1', 'p2'],  # Упрощенно: сумма до = сумма после
        'calculation': 'p1 + p2',
        'units': {'p1': 'кг·м/с', 'p2': 'кг·м/с', 'result': 'кг·м/с'},
        'note': 'Сумма импульсов до столкновения'
    },
    
    # ========== РАБОТА, МОЩНОСТЬ, ЭНЕРГИЯ ==========
    'Работа': {
        'formula': 'A = F × s × cos(α)',
        'parameters': ['F', 's', 'alpha'],  # ← alpha в градусах!
        'calculation': 'F * s * cos(radians(alpha))',  # ← radians!
        'units': {'F': 'Н', 's': 'м', 'alpha': '°', 'result': 'Дж'},
        'note': 'α - угол между силой и перемещением'
    },
    
    'Мощность': {
        'formula': 'P = A / t',
        'parameters': ['A', 't'],
        'calculation': 'A / t',
        'units': {'A': 'Дж', 't': 'с', 'result': 'Вт'}
    },
    
    'Кинетическая энергия': {
        'formula': 'E_k = (m × v²) / 2',
        'parameters': ['m', 'v'],
        'calculation': '0.5 * m * v**2',
        'units': {'m': 'кг', 'v': 'м/с', 'result': 'Дж'}
    },
    
    'Потенциальная энергия': {
        'formula': 'E_p = m × g × h',
        'parameters': ['m', 'g', 'h'],
        'calculation': 'm * g * h',
        'units': {'m': 'кг', 'g': 'м/с²', 'h': 'м', 'result': 'Дж'}
    },
    
    'Коэффициент полезного действия': {
        'formula': 'η = (A_полезн / A_затрач) × 100%',
        'parameters': ['A_pol', 'A_zat'],
        'calculation': '(A_pol / A_zat) * 100',
        'units': {'A_pol': 'Дж', 'A_zat': 'Дж', 'result': '%'}
    },
    
    # ========== ТЕРМОДИНАМИКА ==========
    'Первый закон термодинамики': {
        'formula': 'ΔU = Q - A',
        'parameters': ['Q', 'A'],
        'calculation': 'Q - A',
        'units': {'Q': 'Дж', 'A': 'Дж', 'result': 'Дж'}
    },
    
    'Количество теплоты': {
        'formula': 'Q = c × m × Δt',
        'parameters': ['c', 'm', 'delta_t'],
        'calculation': 'c * m * delta_t',
        'units': {'c': 'Дж/(кг·°C)', 'm': 'кг', 'delta_t': '°C', 'result': 'Дж'}
    },
    
    'Работа идеального газа': {
        'formula': 'A = p × ΔV',
        'parameters': ['p', 'delta_V'],
        'calculation': 'p * delta_V',
        'units': {'p': 'Па', 'delta_V': 'м³', 'result': 'Дж'}
    },
    
    # ========== ЭЛЕКТРОСТАТИКА ==========
    'Закон Кулона': {
        'formula': 'F = k × |q₁ × q₂| / r²',
        'parameters': ['k', 'q1', 'q2', 'r'],
        'calculation': 'k * abs(q1 * q2) / (r**2)',
        'units': {'k': 'Н·м²/Кл²', 'q1': 'Кл', 'q2': 'Кл', 'r': 'м', 'result': 'Н'}
    },
    
    'Напряженность электрического поля': {
        'formula': 'E = F / q',
        'parameters': ['F', 'q'],
        'calculation': 'F / q',
        'units': {'F': 'Н', 'q': 'Кл', 'result': 'Н/Кл (В/м)'}
    },
    
    'Электрическое напряжение': {
        'formula': 'U = A / q',
        'parameters': ['A', 'q'],
        'calculation': 'A / q',
        'units': {'A': 'Дж', 'q': 'Кл', 'result': 'В'}
    },
    
    # ========== ЭЛЕКТРИЧЕСКИЙ ТОК ==========
    'Закон Ома': {
        'formula': 'I = U / R',
        'parameters': ['U', 'R'],
        'calculation': 'U / R',
        'units': {'U': 'В', 'R': 'Ом', 'result': 'А'}
    },
    
    'Работа электрического тока': {
        'formula': 'A = I × U × t',
        'parameters': ['I', 'U', 't'],
        'calculation': 'I * U * t',
        'units': {'I': 'А', 'U': 'В', 't': 'с', 'result': 'Дж'}
    },
    
    'Сила тока': {
        'formula': 'I = q / t',
        'parameters': ['q', 't'],
        'calculation': 'q / t',
        'units': {'q': 'Кл', 't': 'с', 'result': 'А'}
    },
    
    # ========== МАГНЕТИЗМ ==========
    'Сила Ампера': {
    'formula': 'F = B × I × L × sin(α)',
    'parameters': ['B', 'I', 'L', 'alpha'],
    'calculation': 'B * I * L * sin(radians(alpha))',
    'units': {'B': 'Тл', 'I': 'А', 'L': 'м', 'alpha': '°', 'result': 'Н'},
    'note': 'α - угол между проводником и магнитным полем'
},
    
    'ЭДС индукции': {
        'formula': 'ε = -ΔΦ / Δt',
        'parameters': ['delta_phi', 'delta_t'],
        'calculation': '-delta_phi / delta_t',
        'units': {'delta_phi': 'Вб', 'delta_t': 'с', 'result': 'В'}
    },
    
    'Сила Лоренца': {
        'formula': 'F = |q| × v × B × sin(α)',
        'parameters': ['q', 'v', 'B', 'alpha'],  # ← alpha в градусах!
        'calculation': 'abs(q) * v * B * sin(radians(alpha))',  # ← radians!
        'units': {'q': 'Кл', 'v': 'м/с', 'B': 'Тл', 'alpha': '°', 'result': 'Н'},
        'note': 'α - угол между векторами скорости и магнитной индукции'
    },
    
    # ========== ОПТИКА ==========
    'Закон преломления': {
        'formula': 'n₁ × sin(α) = n₂ × sin(β)',
        'parameters': ['n1', 'alpha', 'n2'],  # ← Находим sin(beta) или beta
        'calculation': 'asin((n1 * sin(radians(alpha))) / n2)',  # ← Находим угол beta в радианах
        'units': {'n1': '', 'alpha': '°', 'n2': '', 'result': '°'},
        'note': 'Вычисляется угол β во второй среде',
    },
    
    'Формула тонкой линзы': {
        'formula': '1/F = 1/d + 1/f',
        'parameters': ['d', 'f'],
        'calculation': '1 / d + 1 / f',
        'units': {'d': 'м', 'f': 'м', 'result': '1/м (диоптрии)'}
    },
    
    'Оптическая длина': {
        'formula': 'L = n × l',
        'parameters': ['n', 'l'],
        'calculation': 'n * l',
        'units': {'n': '', 'l': 'м', 'result': 'м'}
    },
    
    # ========== АТОМНАЯ ФИЗИКА ==========
    'Энергия кванта': {
        'formula': 'E = h × ν',
        'parameters': ['h', 'nu'],
        'calculation': 'h * nu',
        'units': {'h': 'Дж·с', 'nu': 'Гц', 'result': 'Дж'}
    },
    
    'Закон радиоактивного распада': {
        'formula': 'N = N₀ × (1/2)^(t / T)',
        'parameters': ['N0', 'T', 't'],
        'calculation': 'N0 * (0.5)**(t / T)',
        'units': {'N0': 'шт', 'T': 'с', 't': 'с', 'result': 'шт'}
    },
    
    'Второй постулат Бора или правило частот': {
        'formula': 'ν = (E₂ - E₁) / h',
        'parameters': ['E2', 'E1', 'h'],
        'calculation': '(E2 - E1) / h',
        'units': {'E1': 'Дж', 'E2': 'Дж', 'h': 'Дж·с', 'result': 'Гц'}
    },
    
    # ========== ТЕОРИЯ ОТНОСИТЕЛЬНОСТИ ==========
    'Релятивистская масса': {
        'formula': 'm = m₀ / √(1 - v²/c²)',
        'parameters': ['m0', 'v', 'c'],
        'calculation': 'm0 / sqrt(1 - (v/c)**2)',
        'units': {'m0': 'кг', 'v': 'м/с', 'c': 'м/с', 'result': 'кг'}
    },
    
    'Полная энергия': {
        'formula': 'E = m × c²',
        'parameters': ['m', 'c'],
        'calculation': 'm * c**2',
        'units': {'m': 'кг', 'c': 'м/с', 'result': 'Дж'}
    },
    
    'Энергия покоя тела': {
        'formula': 'E₀ = m × c²',
        'parameters': ['m', 'c'],
        'calculation': 'm * c**2',
        'units': {'m': 'кг', 'c': 'м/с', 'result': 'Дж'}
    }
}

# Шаг 4: УНИВЕРСАЛЬНАЯ ФУНКЦИЯ ДЛЯ ВСЕХ ФОРМУЛ
def calculate_formula(formula_name, user_values):
    """
    Универсальная функция для расчета ЛЮБОЙ формулы
    
    Как это работает:
    1. Ищем формулу по названию в formula_definitions
    2. Проверяем, что пользователь ввел все нужные параметры
    3. Безопасно вычисляем результат по строке 'calculation'
    4. Возвращаем результат с единицами измерения
    
    formula_name: название формулы (например, 'Закон Ома')
    user_values: словарь с введенными значениями {'U': 10, 'R': 5}
    """
    
    # 1. Проверяем, существует ли такая формула
    if formula_name not in formula_definitions:
        raise ValueError(f"❌ Формула '{formula_name}' не найдена в системе")
    
    formula = formula_definitions[formula_name]
    
    # 2. Проверяем, что все нужные параметры переданы
    missing_params = []
    for param in formula['parameters']:
        if param not in user_values:
            missing_params.append(param)
    
    if missing_params:
        raise ValueError(f"❌ Не хватает параметров: {', '.join(missing_params)}")
    
    # 3. Создаем безопасное окружение для вычислений
    # Это позволяет использовать математические функции (sqrt, sin, cos и т.д.)
    # но запрещает выполнение опасного кода
    safe_env = {
        # Математические функции
        'sqrt': math.sqrt,
        'sin': math.sin, 'cos': math.cos, 'tan': math.tan,
        'asin': math.asin, 'acos': math.acos, 'atan': math.atan,
        'sinh': math.sinh, 'cosh': math.cosh, 'tanh': math.tanh,
        'exp': math.exp, 'log': math.log, 'log10': math.log10,
        # Константы
        'pi': math.pi, 'e': math.e,
        # Преобразования
        'radians': math.radians, 'degrees': math.degrees,
        # Встроенные функции
        'abs': abs, 'round': round
    }
    
    # 4. Добавляем значения пользователя в безопасное окружение
    # Например, user_values = {'U': 10, 'R': 5} станут U=10, R=5 в формуле
    for param, value in user_values.items():
        safe_env[param] = float(value)  # Преобразуем в числа
    
    # 5. Безопасное вычисление результата
    try:
        # eval выполняет строку как код Python, но в ограниченном окружении
        result = eval(formula['calculation'], {"__builtins__": None}, safe_env)
        
        # 6. Формируем красивый ответ
        return {
            'success': True,
            'result': round(result, 4),  # Округляем до 4 знаков
            'formula': formula['formula'],
            'result_with_units': f"{round(result, 4)} {formula['units'].get('result', '')}",
            'units': formula['units'].get('result', ''),
            'calculation_expr': formula['calculation'],
            'parameters_used': {p: user_values[p] for p in formula['parameters']}
        }
        
    except ZeroDivisionError:
        raise ValueError("⚠️ Деление на ноль! Проверьте введенные значения")
    except ValueError as e:
        raise ValueError(f"⚠️ Ошибка в значениях: {str(e)}")
    except Exception as e:
        raise ValueError(f"⚠️ Ошибка при вычислении: {str(e)}")

# Шаг 5: МАРШРУТЫ FLASK (остаются почти без изменений)

@app.route('/')
def home():
    """Главная страница - отображает список формул"""
    return render_template('formulas.html', formulas=formulas)

@app.route('/api/calculate/<formula_name>', methods=['POST'])
def calculate(formula_name):
    """
    API endpoint для расчета формул
    
    Как работает:
    1. Получаем JSON с данными от пользователя
    2. Вызываем универсальную функцию calculate_formula
    3. Возвращаем результат или ошибку в JSON формате
    """
    try:
        # 1. Получаем данные от пользователя
        data = request.get_json()
        
        if not data:
            return jsonify({
                'success': False,
                'error': '❌ Не получены данные для расчета'
            }), 400
        
        # 2. Вычисляем результат с помощью универсальной функции
        result = calculate_formula(formula_name, data)
        
        # 3. Возвращаем успешный результат
        return jsonify(result)
    
    except ValueError as e:
        # Ловим ошибки валидации (не те параметры и т.д.)
        return jsonify({
            'success': False,
            'error': str(e)
        }), 400
    
    except Exception as e:
        # Ловим все остальные ошибки
        return jsonify({
            'success': False,
            'error': f'💥 Внутренняя ошибка сервера: {str(e)}'
        }), 500

# Шаг 6: ДОПОЛНИТЕЛЬНЫЙ API ДЛЯ ФРОНТЕНДА
@app.route('/api/formula-info/<formula_name>', methods=['GET'])
def get_formula_info(formula_name):
    """
    Возвращает информацию о формуле для фронтенда
    (какие параметры нужны, единицы измерения и т.д.)
    """
    if formula_name in formula_definitions:
        formula = formula_definitions[formula_name]
        
        # Формируем подсказки для каждого параметра
        param_hints = {}
        for param in formula['parameters']:
            param_hints[param] = {
                'unit': formula['units'].get(param, ''),
                'description': f'Введите значение {param}'
            }
        
        return jsonify({
            'success': True,
            'formula': formula['formula'],
            'parameters': formula['parameters'],
            'units': formula['units'],
            'param_hints': param_hints,
            'description': formula.get('description', ''),
            'note': formula.get('note', '')
        })
    
    return jsonify({
        'success': False,
        'error': 'Формула не найдена'
    }), 404

# Шаг 7: Запуск приложения
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 Физический калькулятор запущен!")
    print(f"📊 Всего формул в системе: {len(formula_definitions)}")
    print("=" * 50)
    
    # Тестовые примеры (можно убрать в продакшене)
    print("\n🧪 Примеры использования:")
    
    # Тест 1: Закон Ома
    try:
        test_result = calculate_formula('Закон Ома', {'U': 12, 'R': 4})
        print(f"✓ Закон Ома: U=12В, R=4Ом -> I={test_result['result']}А")
    except Exception as e:
        print(f"✗ Ошибка теста: {e}")
    
    # Тест 2: Кинетическая энергия
    try:
        test_result = calculate_formula('Кинетическая энергия', {'m': 2, 'v': 10})
        print(f"✓ Кинетическая энергия: m=2кг, v=10м/с -> E={test_result['result']}Дж")
    except Exception as e:
        print(f"✗ Ошибка теста: {e}")
    
    print("=" * 50)
    print("🌐 Сервер запущен: http://localhost:5000")
    print("=" * 50)
    
    app.run(debug=True, host='0.0.0.0', port=5000)
